  ...
  if (sleepEntries.length >= 3) {
    ...
    if (avgSleep < 7) {
      ...
      alerts.push({
        ...
        message: `Moyenne de ${avgSleep.toFixed(1)}h de sommeil. C'est un peu court pour toi — normal si la faim est plus présente.`,
  ...
  if (stressEntries.length >= 3) {
    ...
    if (avgStress >= 6) {
      ...
      alerts.push({
        ...
        message: `Période stressante (${avgStress.toFixed(1)}/10 en moyenne). On va adapter ensemble pour que ce soit plus facile.`,
